...
# This file has been modified by Jonah Winninghoff - Gallaudet University (Nov 19, 2025)
...
def slowfast():
    cfg = yaml.load(open('slowfast_modules/configs/SLOWFAST_8x8_R50.yaml', 'r'), Loader=yaml.FullLoader)
    cfg = CfgNode(cfg)
    model = SlowFast(cfg)
    return model
...
def visualize():
    ...
    model.eval()
    dummy_input = torch.randn(2, 3, 208, 224, 224)
    os.mkdir('workdir') if not os.path.exists('workdir') else None
    os.mkdir('workdir/onnx') if not os.path.exists('workdir/onnx') else None
    torch.onnx.export(
        model,
        dummy_input,
        "workdir/onnx/slowfast.onnx",
        export_params=True,
        opset_version=18,
        input_names=['input'],
        output_names=['output'],
